...
/* Pufferfish (same as previous build, omitted here for brevity to keep file compact) */
const Breath=(()=>{let raf=null,t=0,run=false;const c=document.getElementById('puffer'),x=c.getContext('2d');
function draw(p){const w=c.width,h=c.height;x.clearRect(0,0,w,h);let g=x.createRadialGradient(w/2,h*.45,20,w/2,h*.6,180);g.addColorStop(0,'#26c6ff');g.addColorStop(1,'#0b5370');x.fillStyle=g;x.fillRect(0,0,w,h);const R=60+40*p,cx=w/2,cy=h/2;x.fillStyle='#ffdf7e';x.strokeStyle='#1a1a1a';x.lineWidth=2;x.beginPath();x.arc(cx,cy,R,0,Math.PI*2);x.fill();x.stroke();x.fillStyle='#ffd166';x.beginPath();x.moveTo(cx+R,cy);x.quadraticCurveTo(cx+R+18, cy-8, cx+R+10, cy-24);x.lineTo(cx+R+26,cy);x.lineTo(cx+R+10, cy+24);x.quadraticCurveTo(cx+R+18, cy+8, cx+R, cy);x.fill();x.beginPath();x.moveTo(cx-R*0.2, cy+R*0.2);x.quadraticCurveTo(cx, cy+R*0.5, cx+R*0.2, cy+R*0.2);x.fillStyle='#ffcc80';x.fill();x.beginPath();x.arc(cx-18, cy-10, 8, 0, Math.PI*2);x.fillStyle='#fff';x.fill();x.beginPath();x.arc(cx-18, cy-10, 4, 0, Math.PI*2);x.fillStyle='#0b1520';x.fill();for(let a=0;a<Math.PI*2;a+=Math.PI/10){const r=R+8+Math.sin(t*0.01+a)*2;const xx=cx+Math.cos(a)*r,yy=cy+Math.sin(a)*r;x.beginPath();x.arc(xx,yy,2,0,Math.PI*2);x.fillStyle='#ffe8a8';x.fill();}x.beginPath();x.arc(cx+20, cy+6, 8+p*4, 0, Math.PI*2);x.fillStyle='#ffb703';x.fill();}
function loop(ts){if(!run)return;t=ts;const cyc=12000,m=t%cyc;let p=0;if(m<4000){p=m/4000}else if(m<6000){p=1}else{p=1-((m-6000)/6000)};draw(p);raf=requestAnimationFrame(loop);}return {start(){if(run)return;run=true;raf=requestAnimationFrame(loop);},stop(){run=false;cancelAnimationFrame(raf);draw(0);} };})();

/* Diver (illustrated style) */
...
function createFishLoop(){
  ...
  function spawnFish(){
    ...
    const bodyColor=randomColor(); fish.innerHTML=fishSVG(bodyColor);

    // size relative to diver (diver ~88x64): 20% - 200%
    ...
    fish.addEventListener('click',()=>{
      if(!fish.classList.contains('photographed')){
        fish.classList.add('photographed'); photos++; document.getElementById('photos').textContent=photos;
      }
    });
    face(); fish.style.left=x+'px'; fish.style.top=y+'px'; layer.appendChild(fish); requestAnimationFrame(move);
  }
  stopFishLoop();
  fishTimer=setInterval(spawnFish, 1100);
}
